...
@when(u'User selects the "{item_number}" item from the list')
def step_impl(context, item_number):
    context.home_page.select_item(item_number)
...
def step_impl(context):
    ...
    assert context.price_1 == context.total_price, (f"price is not same, expected price is {context.price_1} | "
                                                    f"actual price is {context.total_price}")
...
def step_impl(context):
    ...
    assert context.price_1 == context.subtotal, (f"price is not same, expected price is {context.price_1} | "
                                                 f"actual price is {context.subtotal}")
